...
window.onload = function() {
	...
	for (role_update_btn of role_update_btns) {
		...
		role_update_btn.onclick = function(e) {
			...
			axios.get(`/portfolio/member/updateRoles/${id}/roleUser/${roleUser.checked}/roleAdmin/${roleAdmin.checked}`)
	...
	for (enabledUpdateBtn of enabledUpdateBtns) {
		...
		enabledUpdateBtn.onclick = (e) => {
			...
			axios.get(`/portfolio/member/changeMemberState/${id}/${enabled}`)
				 .then(function(response) {
		...
		updateBtn.onclick = (e) => {
			...
			if (mobileCheckFlag == true) { // 유효한 휴대폰 정보라면...
				...
				if (mobileDuplicatedCheckFlag == false) { // 중복된 휴대폰이 없으면...
					...
					axios({
							method : "post",
							url : '/portfolio/member/updateMemberByAdmin',
							data : {
								id : id,	
	...
	for (deleteBtn of deleteBtns) {
		...
		deleteBtn.onclick = (e) => {
			...
			if (confirm(`${id} 님의 회원 정보를 정말 삭제하시겠습니까?`) == true) {
				...
				axios.get(`/portfolio/member/deleteMemberByAdmin/${id}`)
				 .then(function(response) {
