...
public class ProductSearchController {
	...
	public String list(@RequestParam(value="currPage", defaultValue="1") int currPage,
		...
		int productCount = productService.selectProductsCountBySearching(searchKey, searchWord.trim());
		
		productList = productService.selectProductsBySearching(currPage, limit, searchKey, searchWord.trim());	
		
		// 총 페이지 수
		int maxPage = PageVO.getMaxPage(productCount, limit);
		// 시작 페이지 수
		int startPage = 1;
		// 마지막 페이지 수
		int endPage = maxPage;
		...
		pageVO.setListCount(productCount);
